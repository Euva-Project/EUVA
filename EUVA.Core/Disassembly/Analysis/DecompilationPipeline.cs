...
public sealed class DecompilationPipeline
{
    ...
    private static void CleanupIr(IrBlock[] blocks)
    {
        for (int bi = 0; bi < blocks.Length; bi++)
        {
            ...
            if (block.IsEntry)
                FilterPrologue(instrs);

            if (block.IsReturn)
                FilterEpilogue(instrs);
            ...
            foreach (var instr in instrs)
            {
                    ...
                    instr.Sources[0].Kind == IrOperandKind.Register)
                {
                    ...
                    if ((dc == Register.RBP && sc == Register.RSP) ||
                        (dc == Register.RSP && sc == Register.RBP))
                    {
                        instr.IsDead = true;
                    }
                }
    ...
    private static void FilterPrologue(List<IrInstruction> instrs)
    {
        ...
        for (int i = 0; i < Math.Min(instrs.Count, 8); i++)
        {
            ...
            switch (instr.Opcode)
            {
                ...
                {
                    var dc = IrOperand.GetCanonical(instr.Destination.Register);
                    if (dc == Register.RBP || dc == Register.RSP)
                        instr.IsDead = true;
                    break;
                }
        ...
        for (int i = instrs.Count - 1; i >= Math.Max(0, instrs.Count - 8); i--)
        {
            ...
            switch (instr.Opcode)
            {
                case IrOpcode.Load when instr.Destination.Kind == IrOperandKind.Register:
                {
                    ...
                    var dc = IrOperand.GetCanonical(instr.Destination.Register);
                    if (dc == Register.RBP || IsCalleeSaved(dc))
                        instr.IsDead = true;
                    break;
                }
                ...
                {
                    var dc = IrOperand.GetCanonical(instr.Destination.Register);
                    if (dc == Register.RSP || dc == Register.RBP)
                        instr.IsDead = true;
                    break;
                }
            }
    ...
    private static void FilterPushPop(List<IrInstruction> instrs)
    {
        ...
        foreach (var instr in instrs)
        {
            ...
            {
                var sc = IrOperand.GetCanonical(instr.Sources[0].Register);
                if (IsCalleeSaved(sc) || sc == Register.RBP)
                    instr.IsDead = true;
            }
    ...
    private static void OptimizeBlockLocally(IrBlock block)
    {
        ...
        foreach (var instr in instrs)
        {
                ...
                instr.Condition == IrCondition.None)
            {
                ...
                if (canon != Register.None)
                    regConstants[canon] = instr.Sources[0].ConstantValue;
            }
        ...
        foreach (var instr in instrs)
        {
                ...
                instr.Condition == IrCondition.None)
            {
                var canon = IrOperand.GetCanonical(instr.Destination.Register);
                if (canon != Register.None)
                    regCopies[canon] = instr.Sources[0];
            }
            else if (instr.DefinesDest && instr.Destination.Kind == IrOperandKind.Register)
            {
                var canon = IrOperand.GetCanonical(instr.Destination.Register);
        ...
        foreach (var instr in instrs)
        {
            ...
            if (instr.Opcode == IrOpcode.Assign &&
                ...
                instr.Condition == IrCondition.None)
            {
                if (IrOperand.GetCanonical(instr.Destination.Register) ==
                    IrOperand.GetCanonical(instr.Sources[0].Register))
                {
                    instr.IsDead = true;
                }
            }
        ...
        foreach (var instr in instrs)
        {
            ...
            if ((instr.Opcode == IrOpcode.Add || instr.Opcode == IrOpcode.Sub) &&
                ...
                instr.Sources[1].ConstantValue == 0)
            {
                if (instr.Destination.SameLocation(instr.Sources[0]))
                    instr.IsDead = true;
            }
    ...
    private static void ResolveBranchTargets(IrBlock[] blocks)
    {
        ...
        for (int i = 0; i < blocks.Length; i++)
            addrToBlock[blocks[i].StartAddress] = i;
    ...
    private void RecoverCallArguments(IrBlock[] blocks)
    {
        foreach (var block in blocks)
        {
            for (int i = 0; i < block.Instructions.Count; i++)
            {
                ...
                var args = CallingConventionAnalyzer.RecoverCallArguments(block, i);
                if (args.Length > 0)
                {
                    ...
                    {
                        newSources[j + 1] = IrOperand.Reg(args[j].SourceRegister, 64);
                        newSources[j + 1].SsaVersion = -1;
                        newSources[j + 1].Name = args[j].Name;
                    }
    ...
    private static bool IsStackPtr(in IrOperand op)
    {
        ...
        {
            return IrOperand.GetCanonical(op.MemBase) == Register.RSP;
        }
        return op.Kind == IrOperandKind.Register &&
               IrOperand.GetCanonical(op.Register) == Register.RSP;
    }

    private static bool IsCalleeSaved(Register canonical) =>
        canonical is Register.RBX or Register.R12 or Register.R13
            or Register.R14 or Register.R15 or Register.RSI or Register.RDI;
}
